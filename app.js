...
const state=JSON.parse(localStorage.getItem(K)||'{"transactions":[],"brokers":[],"quotes":{},"settings":{}}');
state.transactions||=[];state.brokers||=[];state.quotes||={};state.settings||={};state.marketQuotes||={};state.brokerSnapshots||={};state.migrations||={};
let market='ALL',brokerFilter='ALL',view='STOCK',searchText='',expanded='';
...
function seed(){
  ...
  seedOne('富邦證券（複委託）','US',[['NVDA','輝達',1,203.44,214.72],['UBER','優步',1,72.89,78.60]],'fubon-',31.872);
  const qs=[['fubon-qqqm-rec-1',.53873,283.41,152.83,4964],['fubon-qqqm-rec-2',.51617,300.06,155.03,4951]];
  for(const[id,qty,price,usd,twd]of qs)if(!state.transactions.some(t=>t.id===id))state.transactions.push({id,market:'US',type:'BUY',symbol:'QQQM',name:'Invesco NASDAQ 100 ETF',broker:'富邦證券（複委託）',date:'2026-08-22',qty,price,fee:0,fx:twd/usd});
  seedOne('永豐證券（複委託）','US',[['AAPL','蘋果',2,137.27,309.35],['DIS','華特迪士尼',3,198.87,107.78],['NVDA','輝達',1,202.20,214.72],['QQQM','Invesco NASDAQ 100 ETF',2,296.44,293.76]],'sinopac-',31.2);
  seedOne('Charles Schwab','US',[['SOXL','Direxion Daily Semiconductor Bull 3X Shares',13,181.73,120.60],['MRVL','Marvell Technology',2,197.57,237.04],['FLKR','Franklin FTSE South Korea ETF',1,58,58.50],['AAPL','Apple',1,267,309.35],['SNOW','Snowflake Class A',1,237,332.78],['HPE','Hewlett Packard Enterprise',11,57.28,53.45],['TQQQ','ProShares UltraPro QQQ',12,72.69,71.17],['NVDA','NVIDIA',4,210.49,214.72]],'schwab-',31.2);
  state.migrations.v2ExactV1Restore='2026-08-23';save();
}
seed();
...
function lots(){
  ...
  for(const t of state.transactions){
    ...
    const broker=brokerLabel(t.broker),k=[t.market,t.symbol,broker].join('|');
    if(!m[k])m[k]={market:t.market,symbol:t.symbol,name:t.name,broker,qty:0,cost:0};
    m[k].qty+=+t.qty||0;m[k].cost+=(+t.qty||0)*(+t.price||0)+(+t.fee||0);
  }
